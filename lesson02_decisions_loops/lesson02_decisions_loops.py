...
budget = 14
cost = 15
print("Cost within budget?", cost <= budget)   # False → not feasible
print()
...
if raining:
    # If it is raining, short distances might be walkable,
    # but longer ones use the bus.
    if distance < 0.75:
        print("Walk")
    else:
        print("Bus")
else:
    # If it is not raining, use the distance rules.
    if distance < 0.75:
        print("Walk")
    elif distance < 2.5:
        print("Bike")
...
for s in scores:
    total += s      # add current score to total
    count += 1      # count how many scores we have seen
...
best_time = float('inf')   # start with something very large
best_idx = -1
...
best_fun = -float('inf')   # start with something very small
best_idx = -1
...
best_price = -1   # -1 means "no feasible option yet"
...
# ---------------------------------------------------------
# 9. Practice suggestions (for students)
# ---------------------------------------------------------
...
#
# Students can copy those tasks into a new file:
#     lesson02_practice_problems.py
# and try to solve them before looking at any solutions.
